...
class Net(nn.Module):
    def __init__(self, base_filter, feat, num_stages, n_resblock, scale_factor, pretrained=True, freeze=False):
        ...
        t_net1 = [
            ...
            for _ in range(2)]
        t_net1.append(ConvBlock(feat*5, feat*2, 3, 1, 1, activation='lrelu', norm=None))
        self.t_net_lr = nn.Sequential(*t_net1)
    ...
    def forward(self, t_im1, t_im2, t_flow_f, t_flow_b, train=True):
        ...
        L = L + self.relu_bp(L - self.t_net_lr(torch.cat((L1,L2,M),1)))
